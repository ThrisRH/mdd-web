...
const page = ()=>{
    ...
    const handleGetCate = async ()=>{
        setLoading(true);
        try {
            const response = await fetch("http://localhost:1337/api/blogs/".concat(params.slug, "?populate[subContent][populate]=*&populate[optionImage][populate]=*&populate=cover"), {
                method: "GET"
            });
            const data = await response.json();
            if (!response.ok) {
...
_s(page, "Cd7r1itpTTBjuTLFLaGmiLB/CYw=", false, function() {
    return [
...
//# sourceMappingURL=src_53c9ad85._.js.map