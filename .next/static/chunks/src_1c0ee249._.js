...
;
const ButtonContainer = __TURBOPACK__imported__module__$5b$project$5d2f$node_modules$2f$styled$2d$components$2f$dist$2f$styled$2d$components$2e$browser$2e$esm$2e$js__$5b$app$2d$client$5d$__$28$ecmascript$29$__["default"].button(_templateObject(), (props)=>props.$primary ? "#EA8E31" : "#F1DBC4");
...
;
const Button = (param)=>{
    let { children, primary, onClickFunc } = param;
...
const PostCard = (param)=>{
    ...
    const handleGetInfo = async ()=>{
        ...
        try {
            ...
            console.log("Cover url:", data.data.cover.url);
        } catch (error) {
            console.log("Error: " + error);
        } finally{
            setLoading(false);
...
const page = ()=>{
    ...
    const handleGetCate = async ()=>{
        setLoading(true);
        try {
            const response = await fetch("http://localhost:1337/api/cates/".concat(params.slug, "?populate=*"));
            const data = await response.json();
            if (!response.ok) {
                setLoading(false);
                return;
...
_s(page, "XFvA68p8NRRsXY0+v6xzDijnDZk=", false, function() {
    return [
...
//# sourceMappingURL=src_1c0ee249._.js.map