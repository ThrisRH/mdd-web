...
const PostDetail = (param)=>{
    let { documentId: string, title, publishedAt, mainContent, cover, subContent } = param;
...
"[project]/src/app/topic/[id]/page.tsx [app-client] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
const page = ()=>{
    ...
    const handleGetCate = async ()=>{
        setLoading(true);
        try {
            const response = await fetch("http://localhost:1337/api/blogs/".concat(params.id, "?populate[subContent][populate]=*"));
            const data = await response.json();
            if (!response.ok) {
                setLoading(false);
                return;
...
//# sourceMappingURL=src_b780d66a._.js.map