...
;
;
const PostDetail = ({ documentId: string, title, publishedAt, mainContent, cover, subContent })=>{
...
"[project]/src/app/topic/[id]/page.tsx [app-ssr] (ecmascript)": ((__turbopack_context__) => {
"use strict";
...
const page = ()=>{
    ...
    const handleGetCate = async ()=>{
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:1337/api/blogs/${params.id}?populate[subContent][populate]=*`);
            const data = await response.json();
            if (!response.ok) {
                setLoading(false);
                return;
...
//# sourceMappingURL=src_f8da669c._.js.map