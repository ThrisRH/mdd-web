...
function Home() {
    ...
    const handleGetPost = async (pageNumber)=>{
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:1337/api/blogs?pagination[page]=${pageNumber}&pagination[pageSize]=1&populate=*`);
            const data = await response.json();
...
//# sourceMappingURL=src_42fd36cd._.js.map