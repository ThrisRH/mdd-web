...
const page = ()=>{
    ...
    const handleGetCate = async ()=>{
        setLoading(true);
        try {
            const response = await fetch(`http://localhost:1337/api/blogs/${params.slug}?populate[subContent][populate]=*&populate[optionImage][populate]=*&populate=cover`, {
                method: "GET"
            });
            const data = await response.json();
            if (!response.ok) {
...
//# sourceMappingURL=%5Broot-of-the-server%5D__fd479f6d._.js.map